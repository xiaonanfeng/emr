...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var age = '${misEmrBasicinfo.age}' / scope;//如果年龄是0，则把年龄和范围制空。
        if (age == 0) {
            age = "";
        }
        custom_options($("#age"), age);//然后相除并四舍五入
        custom_options($("#timeScope"), scope);//先选中范围
        ...
        custom_options($("#ethnic"), '${misEmrBasicinfo.ethnic}' == null || '${misEmrBasicinfo.ethnic}' == "" ? "10" : '${misEmrBasicinfo.ethnic}');
        ...
        custom_options($("#sendToOther"), '${misEmrBasicinfo.sendToOther}');
        ...
        custom_options($("#ecgMonitor"), '${misEmrAe.ecgMonitor}');
        ...
        custom_options($("#ecgLead"), '${misEmrAe.ecgLead}');

        custom_options($("#ohters"), '${misEmrAe.ohters}');
        //custom_options($("#ecgOther"),'${misEmrAe.ecgOther}'==null||'${misEmrAe.ecgOther}'==""?"":'${misEmrAe.ecgOther}');
...
// 		custom_options($("#arrivalVs"),'${misEmrPreaidVs.arrivalVs}');
        ...
        if (preaidStatStr!=""){
            ...
            for (var i = 0; i < preaidStatArr.length; i++) {
                var code = preaidStatArr[i].preaidCode;
                if (code == 66) {
                    custom_options($("#patientStat"), preaidStatArr[i].patientStat);
                }
                custom_options($("#preaidSucceed" + preaidStatArr[i].preaidCode), preaidStatArr[i].preaidSucceed);
                custom_options($("#reason" + preaidStatArr[i].preaidCode), preaidStatArr[i].reason);
        ...
        //医生签名:如果初始化没有值就默认登录用户，如果有值就直接初始化
        if ('${misEmrPreaidVs.doctorSign}' != null && '${misEmrPreaidVs.doctorSign}' != "") {
            custom_options($("#doctorSign"), '${misEmrPreaidVs.doctorSign}');
        } else {
            custom_options($("#doctorSign"), '${sysMemberInfo.id}');
        }
        ...
        //耗材应用
        $("#senceAarListTd").load("${ctx}/misEmrAar.do?method=findmisEmrAarInUse&emrId=" + $("#id").val());

        //病历模板调用，接受后台JSON字符串
        //var cmpltArray = eval('${misEmrCmpltList}');
        //调用模板赋值方法
        //非强制更换
        ///initEmrCmplt(cmpltArray,0);
     ...
     * 病历动态模板
     **/
    function initEmrCmplt(cmpltArray, flag) {
        $("input").each(function () {
            if ($(this).attr("Persistence") != "true") {
                $(this).val('');
            }
        });
        $("select").val('');
        ...
        for (var item in cmpltArray) {
            if (flag == 1) {//如果是1就强制更换所有的
                custom_options($("#" + cmpltArray[item].colname), cmpltArray[item].colvalue);
            } else {//如果是0就更换空的
                if ($("#" + cmpltArray[item].colname).val() == "") {//如果是是空制就自动补充
                    custom_options($("#" + cmpltArray[item].colname), cmpltArray[item].colvalue);
                }
            }
        }
