    ...
    <c:forEach var="list" items="${list}">
        <c:if test="${list.xgcs!=0 }">
            <tr title="${list.emrid }:${list.chiefComplaint}">
                ...
                <td>${list.szfz}</td>
                <td>${list.clid}</td>
                <td><fmt:formatDate value="${list.ccsj }" pattern="yyyy-MM-dd HH:mm:ss" type="date"
                                    dateStyle="long"/></td>
                <td>${list.lxr }</td>
                <td>${list.lxdh }</td>
                ...
                <td>
                    <a href="javascript:void(0)" biaozhi="mdf${list.emrid}" openMode="tabMenu"
                       tab_title="修改记录：${list.name}"
