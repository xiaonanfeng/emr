...
$(document).ready(function () {
    ...
    $("select").each(function (index, element) {
        //$(this).after("<span class='new_select_box' tabindex='-1'><input type='text' readonly class='new_select_input' placeholder='------请选择------'><dl></dl></span>");
        $(this).after("<span class='new_select_box' tabindex='-1'><input type='text' class='new_select_input' placeholder=''><dl></dl></span>");
    ...
    $(document).on('click', 'a', function () {
        ...
        if ($(this).attr("openMode") == "tabMenu") {
            ...
            if (typeof(biaozhi) == "undefined") {
                var now = new Date();
                var LS_BS = "CD_" + now.getDate() + now.getHours() + now.getMinutes() + now.getSeconds()
                $(this).attr("biaozhi", LS_BS);
                var link_id = LS_BS;
                var title_id = LS_BS + "_title";
                var nr_id = title_id + "_nr";
                var nr_url = $(this).attr("link_URL");
                var cece = nr_id.replace("_nr", "");
                //var pID = $(this,window.parent.document).attr("id");
                var pID = window.name;
                var menu_title = $(this).text();
                var top_page_sonK = 0;//总width
                var my_array = [];
                ...
                if (top_page_sonK + 100 < father_width) {
                    ...
                    //创建导航的时候 初始化首个导航
                    //alert(nr_url)
                    //$("Frame_right_box iframe").css("display","none");
                    //window.parent.$("Frame_right_box iframe").hide();
                    $.ajaxSetup({cache: false});
                    window.parent.$("#Frame_tab_ul li").removeClass("Frame_tab_bg02");
    ...
    //列表中创建展示内容
    $(".display_new_tr").click(function () {
        var the_lie = $(this).parent().parent().find("td").length;
        var display_key = $(this).attr("new_tr");
        var timestamp = "AD_" + new Date().getTime();
        if (display_key == "no") {
            /*******需要动态生成的内容****/

            $(".build_new_tr").remove();
            $(".display_new_tr").attr("new_tr", "no");
            var new_NR = "<table width='100%' border='0' class='editForm'><tr> <td class='text_into'>模块数量:</td><td class='text_edit'>65</td><td class='text_into'>接口类型:</td><td class='text_edit'>SFP</td><td class='text_into'>接口数量:</td><td class='text_edit'>223</td></tr><tr> <td class='text_into'>已使用:</td><td class='text_edit'>56</td><td class='text_into'>未使用:</td><td class='text_edit'>32</td></tr><tr> <td class='text_into'>模块数量:</td><td class='text_edit'>65</td><td class='text_into'>未使用模块数量:</td><td class='text_edit'>43</td> <td class='text_into'>模块数量:</td><td class='text_edit'>65</td></tr><tr> <td class='text_into'>已使用:</td><td class='text_edit'>56</td> <td class='text_into'>模块数量:</td><td class='text_edit'>65</td><td class='text_into'>未使用:</td><td class='text_edit'>32</td></tr></table>";
            ///

            $(this).parent().parent().after("<tr class='build_new_tr'><td id=" + timestamp + " colspan=" + the_lie + " style=''>" + new_NR + "</td></tr>");
            $(this).attr("new_tr", "yes");
        } else {
            $(".build_new_tr").remove();
            $(this).attr("new_tr", "no");
        }
